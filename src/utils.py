...
def eye_refinement(img, img_name=None):
    def re_label(img, value):
        ...
        idx = 0
        prev_idx = 0
        for props in measure.regionprops(mask_regions, img_filled):
            idx = idx + 1
            if props.area > ld_area:
                ld_area = props.area
    ...
    img_filled_sclera = re_label(img, value=0)
    img_sclera = img.copy()
    img_sclera[img_filled_sclera == 0] = 0  # Largest region, consider as eye

    img_filled_iris = re_label(img_sclera, value=1)
    img_iris = img_sclera.copy()
    img_iris[img_filled_iris == 0] = 0

    img_filled_pupil = re_label(img_iris, value=2)
